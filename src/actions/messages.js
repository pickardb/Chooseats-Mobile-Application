...
export const sendMessage2 = (values, roomId) => async (dispatch) => {
    const { text } = values;
    ...
    try {
        await dispatch({
            type: messageTypes.SEND_MESSAGE,
            payload: messagesService.create({ roomId: roomId, text: values[0].text, type: "user" })
        });
        //Disabled for now
        //dispatch(removeLocalMessage(localMessageId));
    }
...
export const getMessages = (id) => async dispatch => {
    try {
        await dispatch({
            ...
            payload: messagesService.find({ query: { roomId: id, $sort: {id:-1}, $limit: 30 } })
        });
    }
    catch (err) {
        console.log(err);
